...
function addAtributo(nome = '', opcoes = []) {
    ...
    div.innerHTML = `<div class="flex gap-2 mb-4"><input type="text" placeholder="Atributo" value="${nome}" class="atrib-nome flex-1 font-bold p-2 border-b-2 border-indigo-50 outline-none"><button onclick="this.parentElement.parentElement.remove()" class="text-red-300">✕</button></div><div class="lista-opcoes space-y-2"></div><button type="button" class="btn-add-op mt-4 text-[10px] font-bold uppercase text-indigo-400">+ Add Opção</button>`;
...
function addLinhaPacote(q='', p='') {
    ...
    div.innerHTML = `<input type="number" placeholder="Qtd" value="${q}" class="q w-full p-2 border rounded"><input type="number" placeholder="Preço Total" value="${p}" class="p w-full p-2 border rounded font-bold text-amber-600"><button onclick="this.parentElement.remove()" class="text-red-300">✕</button>`;
...
function addLinhaProgressivo(q='', p='') {
    const div = document.createElement('div');
    div.className = "flex gap-2";
    div.innerHTML = `<input type="number" placeholder="Qtd Mín" value="${q}" class="q w-full p-2 border rounded"><input type="number" placeholder="Preço Unit" value="${p}" class="p w-full p-2 border rounded font-bold text-emerald-600"><button onclick="this.parentElement.remove()" class="text-red-300">✕</button>`;
...
async function salvarProduto() {
    ...
    const d = {
        ...
        prazo: document.getElementById('prodPrazo').value || 0,
...
function calcularPrecoAoVivo() {
    ...
    if(regra === 'm2') {
        ...
        if(p.larguraBobina > 0 && menorLado > p.larguraBobina) aviso.classList.remove('hidden'); else aviso.classList.add('hidden');
        m2 = l * a; totalBase = (base + extraVar) * m2 * qtd;
    } else if(regra === 'pacote') {
...
function renderAcabTable() { document.getElementById('listaAcabamentosTab').innerHTML = bdAcabamentos.map(a => `<tr class="border-b border-slate-50"><td class="p-5 font-bold">${a.nome} (${a.grupo})</td><td class="p-5 text-center"><button onclick="db.collection('acabamentos').doc('${a.id}').delete()" class="text-red-300">✕</button></td></tr>`).join(''); }
function renderFiltrosVitrine() { const div = document.getElementById('menuFiltroCat'); if(!div) return; div.innerHTML = `<button onclick="renderVitrine('Todos')" class="px-5 py-2 bg-white border border-slate-100 rounded-full font-bold text-xs hover:bg-indigo-600 hover:text-white transition shadow-sm">Todos</button>` + bdCategorias.map(c => `<button onclick="renderVitrine('${c.nome}')" class="px-5 py-2 bg-white border border-slate-100 rounded-full font-bold text-xs hover:bg-indigo-600 hover:text-white transition shadow-sm">${c.nome}</button>`).join(''); }
function renderPedidosFinanceiro() { const tab = document.getElementById('listaPedidosTab'); if(!tab) return; tab.innerHTML = bdPedidos.map(p => `<tr class="border-b border-slate-50 hover:bg-slate-50 transition"><td class="p-5 text-slate-400 font-medium">${p.data.toDate().toLocaleDateString('pt-BR')}</td><td class="p-5 font-bold text-slate-700">${p.clienteNome}</td><td class="p-5 font-black text-indigo-600">R$ ${p.total.toFixed(2)}</td><td class="p-5 text-center"><span class="bg-indigo-50 text-indigo-500 px-3 py-1 rounded-full text-[10px] font-black uppercase">${p.status}</span></td></tr>`).join(''); }
function verHistoricoCliente(idCli) { const cliente = bdClientes.find(x => x.id === idCli); const pedidosCli = bdPedidos.filter(p => p.clienteId === idCli); document.getElementById('histNomeCli').innerText = `Pedidos de: ${cliente.nome}`; const corpo = document.getElementById('corpoHistoricoCli'); corpo.innerHTML = pedidosCli.length === 0 ? "<p class='text-center text-slate-400 py-10'>Nenhum pedido.</p>" : pedidosCli.map(p => `<div class="bg-slate-50 p-4 rounded-2xl border border-slate-100"><div class="flex justify-between font-bold text-indigo-900 mb-2"><span>${p.data.toDate().toLocaleDateString('pt-BR')}</span><span>R$ ${p.total.toFixed(2)}</span></div><div class="text-xs text-slate-500">${p.itens.map(i => `• ${i.nome}`).join('<br>')}</div></div>`).join(''); document.getElementById('modalHistoricoCli').classList.remove('hidden'); }
...
function renderCarrinho() { const div = document.getElementById('listaCarrinho'); if(!div) return; let sub = 0; div.innerHTML = carrinho.map((item, i) => { sub += item.valor; return `<div class="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100"><div class="max-w-[70%] text-xs font-bold text-slate-800">${item.nome}<br><span class="text-[9px] font-medium text-slate-400">${item.desc}</span></div><div class="text-right"><p class="font-black text-indigo-600 text-sm">R$ ${item.valor.toFixed(2)}</p><button onclick="carrinho.splice(${i},1);renderCarrinho()" class="text-[9px] font-bold text-red-400 uppercase">Remover</button></div></div>`; }).join(''); document.getElementById('subtotalCart').innerText = "R$ " + sub.toFixed(2); atualizarTotalFinal(); }
...
function atualizarListaAcabamentosProduto(salvos = []) { const container = document.getElementById('listaCheckAcabamentos'); if(!container) return; const cat = document.getElementById('prodCategoria').value; const filtrados = bdAcabamentos.filter(a => a.categoria === cat || a.categoria === "Geral"); container.innerHTML = filtrados.map(a => { const obj = salvos.find(s => (s.id || s) === a.id); const checked = obj ? 'checked' : ''; const starAtiva = (obj && obj.padrao) ? 'text-amber-400' : 'text-slate-200'; return `<div class="flex items-center justify-between p-2 bg-white border rounded"><label class="text-[10px] font-bold flex items-center gap-2 cursor-pointer"><input type="checkbox" class="check-acab-prod" value="${a.id}" ${checked}> ${a.nome}</label><i class="fa fa-star cursor-pointer star-padrao ${starAtiva}" onclick="this.classList.toggle('text-amber-400'); this.classList.toggle('text-slate-200')"></i></div>`; }).join(''); }
